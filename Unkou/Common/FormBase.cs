    ...
    public partial class FormBase : Form
    {
		...
		protected void FormBase_Load(object sender, EventArgs e)
		{
			try
			{
				...
				if (this.DesignMode)
				{
					...
					usingFlgCheckCount = 0;
					usingFlgCheckInterval = 0;
					CommonProcess.XmlFilePath = string.Empty;
					CommonProcess.XmlFilePath_Guide = string.Empty;
					CommonProcess.XmlFilePath_Screen = string.Empty;
					this.setFormId = string.Empty;
					this.setTitle = string.Empty;
					this.setWindowTitle = string.Empty;
				}
				else
				{
					// イベント前処理
					...
					//TODO:Loadの共通独自処理を実行したければここに記入
					ReadAppConfig reaAppConfig = new ReadAppConfig();
					string exeOpenerPath = System.Convert.ToString(System.AppDomain.CurrentDomain.BaseDirectory);

					// 接続文字列の取得
					if (connectionString == string.Empty)
					{
						if (SystemInfoManagement.testFlg == TestFlgType.normalUser)
						{
							connectionString = reaAppConfig.getConnectString(TestFlgType.normalUser);
						}
						else if (SystemInfoManagement.testFlg == TestFlgType.testUser)
						{
							connectionString = reaAppConfig.getConnectString(TestFlgType.testUser);
						}

					}

					// メッセージファイルの親パスの取得
					if (CommonProcess.XmlFilePath == string.Empty)
					{
						CommonProcess.XmlFilePath = exeOpenerPath
							+ reaAppConfig.getAppSetting("MessageList_Path");
					}

					// ガイドメッセージファイルの親パスの取得
					if (CommonProcess.XmlFilePath_Guide == string.Empty)
					{
						CommonProcess.XmlFilePath_Guide = exeOpenerPath
							+ reaAppConfig.getAppSetting("GuidMessageList_Path");
					}

					// 画面情報ファイルの親パスの取得
					if (CommonProcess.XmlFilePath_Screen == string.Empty)
					{
						CommonProcess.XmlFilePath_Screen = exeOpenerPath
							+ reaAppConfig.getAppSetting("ScreenInfo_Path");
					}

					//ローカルログ出力フラグ設定(0:出力しない、1：日付単位に出力、2:日付画面単位に出力)
					if (reaAppConfig.getAppSetting("LocalLogOutPutFlg") == System.Convert.ToString(LogOut.LocalLogOutPutMode.Day))
					{
						createFactoryLog.LocalLogOutPutFlg = LogOut.LocalLogOutPutMode.Day;
					}
					else if (reaAppConfig.getAppSetting("LocalLogOutPutFlg") == System.Convert.ToString(LogOut.LocalLogOutPutMode.DayGamen))
					{
						createFactoryLog.LocalLogOutPutFlg = LogOut.LocalLogOutPutMode.DayGamen;
					}

					//出力先
					if (createFactoryLog.LocalLogOutPutFlg != LogOut.LocalLogOutPutMode.No)
					{

						if (createFactoryLog.LocalLogOutPutPath == string.Empty)
						{
							createFactoryLog.LocalLogOutPutPath = reaAppConfig.getAppSetting("LocalLogOutPutPath");
						}
					}


					//DBログ出力フラグ設定(True:出力、False：未出力(デフォルト))
					if (reaAppConfig.getAppSetting("DbLogOutPutFlg") == "1")
					{
						createFactoryLog.DbLogOutPutFlg = true;

					}

					// 使用中フラグのチェック回数・チェック間隔取得
					string strUsingFlgCheckCount = string.Empty;
					string strUsingFlgCheckInterval = string.Empty;

					strUsingFlgCheckCount = System.Convert.ToString(reaAppConfig.getAppSetting("UsingFlgCheckCount"));
					strUsingFlgCheckInterval = System.Convert.ToString(reaAppConfig.getAppSetting("UsingFlgCheckInterval"));

					if (Information.IsNumeric(strUsingFlgCheckCount) == true)
					{
						usingFlgCheckCount = strUsingFlgCheckCount;
					}
					if (Information.IsNumeric(strUsingFlgCheckInterval) == true)
					{
						usingFlgCheckInterval = strUsingFlgCheckInterval;
					}
					...
					// 画面情報の取得
					Common.CommonProcess.getScreenInfo(this.Name, fromSystemCd);
					this.setFormId = System.Convert.ToString(Common.CommonProcess.clsScreenInfoGet.ScreenId); // 画面ID
					this.setTitle = System.Convert.ToString(Common.CommonProcess.clsScreenInfoGet.ScreenTitle); // 画面タイトル
					this.menuId = System.Convert.ToString(Common.CommonProcess.clsScreenInfoGet.menuId); // 該当メニューID
					this.systemCd = Common.CommonProcess.clsScreenInfoGet.systemCd; // 該当システムコード

					// ウィンドウタイトル
					this.setWindowTitle = System.Convert.ToString(string.Format(windowTitleString,);
					SystemInfoManagement.systemNm(,);
					UserInfoManagement.sectionBusyoName(,);
					UserInfoManagement.userName());

					// 発行バージョン
					if (string.IsNullOrEmpty(System.Convert.ToString(UserInfoManagement.isueVersion)) == true)
					{
						if (System.Deployment.Application.ApplicationDeployment.IsNetworkDeployed)
						{
							UserInfoManagement.isueVersion = "発行Ver：" + System.Deployment.Application.ApplicationDeployment.CurrentDeployment.CurrentVersion.ToString();
						}
						else
						{
							UserInfoManagement.isueVersion = "製品Ver：" + Application.ProductVersion;
						}
						// システム情報のバージョンも同一のものをセット
						SystemInfoManagement.issueVersion = UserInfoManagement.isueVersion;
					}

					// 権限の設定（各種フラグ、権限レベルのセット）
					if (string.IsNullOrEmpty(System.Convert.ToString(this.systemCd)) == false)
					{
						Common.AuthInfo.setAuth(this.systemCd);
					}

					// 背景色設定（練習環境、本番環境）
					if (UserInfoManagement.testFlg == TestFlgType.normalUser)
					{
						// 本番環境
						this.BackColor = FixedCd.BackgroundColorType.Form;
					}
					else
					{
						// 練習環境
						this.BackColor = FixedCd.BackgroundColorType.TestForm;
					}
					...
					//操作ログ
					createFactoryLog.logOutput(LogKindType.operationLog, ProcessKindType.sonota, setFormId + ":" + setTitle, "フォームロード");
			...
			{
				createFactoryMsg.messageDisp("E90_018", ex.Number);
				string[] strErr;
				strErr = new string[] { ex.Message, ex.Source, ex.StackTrace };
				createFactoryLog.logOutput(LogKindType.fatalLog, ProcessKindType.sonota, setFormId + ":" + setTitle, strErr);
				this.Close();
			}
			catch (Exception ex)
			{
				...
				string[] strErr;
				strErr = new string[] { ex.Message, ex.Source, ex.StackTrace };
				createFactoryLog.logOutput(LogKindType.errorLog, ProcessKindType.sonota, setFormId + ":" + setTitle, strErr);
				this.Close();
			}
			finally
			{
				// フォームロード時の「イベント後処理」の処理はここでは不適切
				// 理由として、FormBaseのLoadイベント後も処理が存在する場合があり
				// 画面の表示処理自体が完了していないため、表示処理の完了後に実行される
				//「Shown」イベントにて実行するのが望ましい
				// その理由として
				// 設計上、各画面パターンや画面個別において、画面表示後の処理の実装が想定
				// されていないため、FormBaseのShownイベントが表示処理におけるの最後の処理
				// と想定されるため
				//'
				//' イベント後処理
				//comPostEvent()
		...
		protected void FormBase_KeyDown(object sender, System.Windows.Forms.KeyEventArgs e)
		{
			try
			{
				...
				else if (e.KeyData == Keys.Pause)
				{
					...
					createFactoryMsg.messageDisp("9999", UserInfoManagement.isueVersion);
				}
				else
				{
					//その他キー
		...
		protected virtual void btnCom_Click(System.Object sender, System.EventArgs e)
		{
			try
			{
				...
				{
					createFactoryLog.logOutput(LogKindType.operationLog, ProcessKindType.sonota, setFormId + ":" + setTitle, ((ButtonEx)sender).Text);
					btnF1_ClickOrgProc();
				}
		...
		protected void openWindow(Form frm, bool parentDispFlg)
		{
			...
			catch (Exception ex)
			{
				...
				createFactoryLog.logOutput(LogKindType.errorLog, ProcessKindType.sonota, setFormId + ":" + setTitle, strErrNaiyo);
				createFactoryMsg().messageDisp("E90_046", ex.Message);
				frm.Close();
			}
			finally
		...
		protected virtual int DbOperator(DbShoriKbn updateFlg)
		{
			...
			if (CommonProcess.createFactoryMsg().messageDisp(msgid, msgParam) == MsgBoxResult.Cancel)
			{
				//キャンセル
				return RET_CANCEL;
			}
